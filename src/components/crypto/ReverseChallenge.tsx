        ...
        {!submitted ? (
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-bg-soft border border-border rounded-xl font-sans text-sm text-text-primary px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-purple/30 focus:border-purple placeholder:text-text-secondary/50 transition-colors"
                ...
                <button
                  type="submit"
                  disabled={!guess.trim()}
                  className="px-5 py-2.5 rounded-xl font-semibold text-sm text-white disabled:opacity-40 disabled:cursor-not-allowed transition-all hover:opacity-90 active:scale-95"
          ...
          <div className="p-5 space-y-4">
            <div className="rounded-2xl bg-green-light border border-green/20 p-4 text-center">
              ...
              <div className="text-green font-bold text-base mb-1">{t("correctTitle")}</div>
              <div className="text-sm text-text-secondary">
                {t("correctBody", { phrase: HIDDEN_PHRASE })}
              </div>
            </div>
            <p className="text-sm text-text-secondary leading-relaxed">{t("correctNote")}</p>
            <button onClick={handleReset} className="text-sm text-text-secondary hover:text-text-primary transition-colors font-medium">
              {t("tryAgain")}
            </button>
          </div>
        ) : (
          <div className="p-5 space-y-4">
            <div className="rounded-2xl bg-red-light border border-red/20 p-4 flex gap-3 items-start">
              ...
              <div>
                <div className="font-bold text-red text-sm">{t("wrongTitle")}</div>
                <div className="text-xs text-text-secondary mt-0.5">{t("wrongSubtitle")}</div>
              </div>
            </div>
            <div className="space-y-3">
              <div className="rounded-xl bg-code-bg border border-code-border p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-text-secondary mb-2">{t("yourHashLabel")}</div>
                <HashDisplay hash={guessHash} diffAgainst={targetHash} />
              </div>
              <div className="rounded-xl bg-code-bg border border-code-border p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-text-secondary mb-2">{t("targetHashLabel")}</div>
            ...
            <div className="rounded-xl bg-blue-light border border-blue/20 px-4 py-3 text-sm">
              ...
              <span className="text-text-secondary">{t("matchCountSuffix")}</span>
            </div>
            <button onClick={handleReset} className="text-sm text-text-secondary hover:text-text-primary transition-colors font-medium">
              {t("tryAgain")}
            </button>
